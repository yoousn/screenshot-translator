...
def parse_items(filepath):
    ...
    while i < len(lines):
        ...
        if line.startswith('use ') or line.startswith('pub use '):
            if not stripped.endswith('*;'):  # skip pub use crate::* because it causes issues
        ...
        if match:
            ...
            if not item_lines[decl_idx].startswith('pub ') and not item_lines[decl_idx].startswith('impl'):
                words = item_lines[decl_idx].split(' ', 1)
...
def main():
    ...
    for item in all_items:
        ...
        if not found:
            # check if it was originally in window_control.rs or something
            # if we didn't map it, keep it in lib.rs
            lib_remaining.extend(item['lines'])
            lib_remaining.append('\n')

    # Fix conflicting generic names in all_uses if any.
    # It's better to manually add what we need, but we'll try with all_uses
    # Exclude internal crate module uses
    clean_uses = []
    ...
    if os.path.exists("window_control.rs"):
        os.remove("window_control.rs")
    if os.path.exists("recording_commands.rs"):
        os.remove("recording_commands.rs")

    print("Refactoring complete with improved parser.")
