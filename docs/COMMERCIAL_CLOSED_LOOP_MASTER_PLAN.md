...
- Chapter 148 已优化服务端翻译慢失败路径：真实 `TranslationCache` 改为 `OrderedDict` LRU，缓存测试直接覆盖生产实现，LLM 批处理失败不再逐条慢 fallback，provider batch/fallback/failure timing 已暴露给客户端。
- Chapter 148 已继续加固翻译覆盖字号：段落分发只匹配到单个 render line 时仍使用原始行锚点，字号估算同时参考框高和原文宽度密度，避免英文/段落译文用异常高框放大。
- Chapter 148 已修复截图误触风险：全局快捷键防抖，截图 payload 携带 mode 避免 normal/translate/record 串台，native pointer recovery 只有真实移动后才接管，确认入口要求稳定有效选区。
...
| 风险 | 当前真实状态 | 下一步处理 |
|---|---|---|
| RapidOCR 主线仍需真实端到端验收 | 生成式 fixture、打包 runner fixture、真实 release `Alt+A` 首击框选、`Ctrl+D` 结果窗、`Alt+A` 最终发布版秒表、四张用户真实截图 OCR fixture、`测试图片\测试2` 三张图 fixture、Chapter 144–147 release smoke 均已通过；Chapter 147 已补 UIA 父容器串台门禁；Chapter 148 已补截图误触状态机防护；仍缺多屏/DPI 和真实覆盖层肉眼 smoke | 补覆盖层视觉回归，继续记录失败样例并修候选评分/后处理 |
...
目标：在 RapidOCR 常驻 worker、截图启动热路径优化、论坛列表 OCR 清洗、正文段落 OCR 合并、行级覆盖渲染分发、翻译预热诊断、UIA 文本源抢跑和 Chapter 148 翻译慢失败/截图误触防护基础上，继续补真实翻译覆盖层验收并把诊断细分暴露给用户；`Alt+A` Chapter 144 发布版 smoke 已观察到截图辅助窗口约 `106ms`，当前公网翻译仍受 RTT/TLS/服务端链路限制。
...
- 保持 Chapter 148 的渲染字号保险和截图确认状态机防护，避免英文/段落译文再次被异常高框放大，避免 `Alt+A` 残留鼠标状态误触确认。
