...
- License posture: **source-available for learning and personal non-commercial use; not an OSI-approved open-source license**
...
For normal use, install from the GitHub Release asset:

1. Open the latest GitHub Release.
2. Expand **Assets**.
3. Download `YsnTrans_<version>_x64-setup.exe`, for example `YsnTrans_1.2.8_x64-setup.exe`.
4. Run the installer and launch **YsnTrans** from the Start menu or desktop shortcut.

The automatically generated **Source code (zip)** and **Source code (tar.gz)** files are only source snapshots. They are not the Windows app installer.

This project currently ships unsigned local builds, so Windows SmartScreen may appear. If you trust this release, use the standard Windows confirmation path for unsigned desktop software.
...
```text
.
├─ app.py                            # Local deploy-helper UI backend
├─ ui.html                           # Local deploy-helper UI
├─ 启动部署助手.bat                  # Local deploy-helper launcher
├─ docs/                             # Product direction and implementation history
├─ ffmpeg/                           # Optional FFmpeg runtime location
├─ models/                           # RapidOCR V5/V4 compatibility model assets
├─ ocrv6/                            # PP-OCRv6 model assets
├─ release/                          # Local assembled release artifacts
├─ scripts/                          # Build, quality, maintenance, and deploy helpers
│  ├─ build/
│  ├─ deploy/
│  ├─ maintenance/
│  └─ quality/
...
The setup installer is generated under:

```text
tauri-client/src-tauri/target/release/bundle/nsis/
```

Upload the generated `YsnTrans_<version>_x64-setup.exe` file to the matching GitHub Release.

Generated setup artifacts are written to:
...
This repository is public for transparency, learning, review, and personal non-commercial use. It is source-available, but it is not released under an OSI-approved open-source license.
