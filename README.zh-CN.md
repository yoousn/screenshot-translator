...
- 授权定位：**源码可见，允许学习和个人非商业使用；不是 OSI 认可意义上的开源许可证**
...
普通用户请从 GitHub Release 的安装包下载：

1. 打开最新 GitHub Release。
2. 展开 **Assets**。
3. 下载 `YsnTrans_<version>_x64-setup.exe`，例如 `YsnTrans_1.2.8_x64-setup.exe`。
4. 运行安装包，然后从开始菜单或桌面快捷方式启动 **YsnTrans**。

GitHub 自动生成的 **Source code (zip)** 和 **Source code (tar.gz)** 只是源码快照，不是 Windows 应用安装包。

当前项目本地构建不做代码签名，因此 Windows SmartScreen 可能弹出提示。如果你信任该版本，请按 Windows 对未签名桌面软件的标准确认流程继续安装。
...
├─ app.py                            # 本地部署助手 UI 后端
├─ ui.html                           # 本地部署助手 UI
├─ 启动部署助手.bat                  # 本地部署助手启动入口
├─ docs/                             # 产品方向与实现历史
├─ ffmpeg/                           # 可选 FFmpeg 运行时位置
├─ models/                           # RapidOCR V5/V4 兼容模型资产
├─ ocrv6/                            # PP-OCRv6 模型资产
├─ release/                          # 本地组装发布产物
├─ scripts/                          # 构建、质检、维护和部署辅助脚本
│  ├─ build/
│  ├─ deploy/
│  ├─ maintenance/
│  └─ quality/
...
把生成的 `YsnTrans_<version>_x64-setup.exe` 上传到对应的 GitHub Release。
...
本仓库公开用于透明展示、学习、审阅和个人非商业使用。它属于 source-available（源码可见），但不是 OSI 认可意义上的开源许可证。

完整条款见 [LICENSE.md](LICENSE.md)。简要来说：可以学习、私人 fork、个人非商业运行；不得重新打包、再分发、转卖、发布品牌化构建、冒充原创，或未经许可使用 YsnTrans 名称、图标和品牌。

本仓库由 YsnTrans 发布者维护。面向广泛公开分发前，应补充支持渠道和隐私说明。
