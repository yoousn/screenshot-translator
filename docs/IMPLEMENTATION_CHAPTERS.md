...
## Chapter 167 - Screenshot Hot Path And Quick Save Latency Optimization (2026-06-08)

### Goal

- Reduce perceived `Alt+A` screenshot startup latency so the overlay appears immediately after the hotkey.
- Reduce `Ctrl+S` save latency after selecting a screenshot region.
- Add concrete performance logging for repeated local verification.
...
- Did not remove the DWM settle/focus reliability path because previous chapters documented regressions when that path was made too passive.
- Did not change OCR, translation, recording, release signing, or installer packaging.
- Did not keep the unstable large-base64 event payload experiment; it was reverted after local testing showed image-ready instability.

### Actual Changes

- Added a pre-capture screenshot loading phase: the hidden screenshot WebView is positioned, marked capture-excluded, shown topmost, and receives `screenshot-loading` before capture/PNG work finishes.
- Made `show_screenshot_overlay_window` reusable from the screenshot command path so the overlay can appear before screenshot image preparation completes.
- Added a lightweight preparing UI state for the screenshot page instead of keeping the window fully transparent until the PNG is decoded.
- Changed the post-capture image handoff to write the PNG backup on a blocking task and emit a small file payload, avoiding a 10MB+ base64 event and avoiding the slower frontend `get_fullscreen_image` IPC path for the common case.
- Added output warming for selected regions so the selected PNG is cropped/rendered shortly after selection instead of on `Ctrl+S`.
- Changed save to quick-save directly into `Pictures/YsnTrans Screenshots` with timestamped filenames, avoiding the system save dialog and its unavoidable latency.
- Removed the 120ms confirm-age delay for save actions while keeping selection/drag safety checks.
- Added `[screenshot-perf]` logs for overlay shown, capture ready, file backup ready, frontend image ready, output warm/cache, and quick-save backend/total timings.
...
- `npm run build` from `tauri-client`: passed. Existing Vite warnings remain: mixed static/dynamic import for `@tauri-apps/api/window.js` and chunk size over `1200 kB`.
- `git diff --check`: passed.
- Local dev smoke via `npm run tauri dev`: passed with synthetic `Alt+A`, drag selection, and native `Ctrl+S` keyboard input.

### Measured Local Latency

- Pre-optimization observation: screenshot overlay appeared only after capture/PNG/base64/frontend decode, roughly 1-2 seconds by user report and code-path inspection.
- First immediate-overlay test after pre-capture show: `Alt+A` visible overlay was 47ms cold, then 10ms, 11ms hot; full screenshot payload still took 1064-1233ms.
- Direct base64 event experiment: overlay remained fast but 10MB+ base64 event delivery was unstable for image-ready behavior, so this route was rejected.
- Memory IPC fallback test after reverting direct base64: overlay 24ms; frontend image ready 1829ms for a 10.6MB PNG.
- File payload optimization test: overlay 25ms, 19ms, 19ms, 21ms, 22ms; capture ready 742ms, 742ms, 689ms, 668ms, 666ms; backup write 4-5ms; frontend image ready 889ms, 909ms, 791ms, 764ms, 767ms.
- Quick save test after selected-output warming: output warmed in 14ms, 20ms, 9ms; cache hit on `Ctrl+S`; backend write 3ms, 2ms, 2ms; total quick-save action 10ms, 10ms, 9ms.
- Saved files verified in `C:\Users\ysn\Pictures\YsnTrans Screenshots` with fresh PNG outputs around 203-206KB for the tested region.

### Current Risks

- The overlay is intentionally excluded from capture, so desktop screenshots used for testing will not visibly show it; rely on real visual inspection and performance logs.
- `Ctrl+S` now quick-saves without a save dialog. This matches the requested near-instant save behavior but changes the old save-as expectation for toolbar save as well.
- Screenshot image ready is still bounded mostly by actual screen capture and image decode; the UI is now immediate, but the full screenshot bitmap typically becomes ready around 0.76-0.91s on this machine.
- Synthetic keyboard automation required native `keybd_event` to hit `Ctrl+S`; ordinary user input should work through the focused overlay path, but real manual smoke remains recommended.

### Next Recommended Chapter

- Add a small ��Saved to ���� toast/action affordance that can open the quick-save folder, and optionally add a separate ��Save As���� advanced/menu action if users still need a dialog-based path.

### Chapter 167 Follow-Up - Save Dialog And Flicker Regression Fix (2026-06-08)

- User reported the pre-capture overlay caused visible flash/stutter and that quick-save felt like no response.
- Reverted the aggressive pre-capture visible overlay path; screenshot overlay now returns to showing only after the screenshot image is ready, while retaining the faster file-payload image handoff.
- Removed the in-screenshot preparing card and `screenshot-loading` frontend listener.
- Replaced quick-save behavior with a native Save dialog for `Ctrl+S` and toolbar save.
- The Save dialog defaults to `dirs::desktop_dir()`, so it follows the operating system's actual Desktop location even when Desktop is redirected to another drive such as D/E/F.
- Removed the screenshot-window success message; save feedback now comes from the native Save dialog workflow instead of an Ant Design message inside the overlay.
- Kept selected-output warming so choosing a file path can happen first and PNG generation/write happens after the user confirms the native dialog.
- Re-ran `cargo check`, `npm run build`, and `git diff --check`: passed. Existing Vite warnings remain unchanged.

### Chapter 168 - Screenshot Latency Plan And Cleanup Gate (2026-06-08)

### Goals

- Stop further implementation and convert the screenshot latency / save / selection work into a gated, step-by-step plan as requested by the user.
- Keep Snow Shot source and portable reference available for future comparison.
- Clean clearly obsolete local clutter while avoiding deletion of release, OCR, deploy, icon, and dependency scripts without user confirmation.
- Register the new screenshotר�� plan in the master plan before adding a separate Markdown document, following the project documentation rules.
...
- Did not implement Chapter A/B/C optimizations yet; the user asked to plan first and gate every implementation step behind manual build/run confirmation.
- Did not delete `C:\Users\ysn\AppData\Local\Temp\snow-shot-src` or the desktop Snow Shot portable directory because the user explicitly wants to keep those source clues.
- Did not delete `.ps1` scripts. The plan lists which scripts should be kept and which require explicit user confirmation before cleanup.
- Did not change OCR, translation, recording, or release behavior in this chapter.

### Validation

- Pending docs-only final check after the plan is written: run `git diff --check` and inspect `git status --short`.
- No build required for this chapter because it only adds/updates planning documents and records already-requested cleanup.

### Next Recommended Chapter

- Ask the user to review `docs/SCREENSHOT_LATENCY_AND_SELECTION_PLAN.md` and confirm whether to add more Snow Shot-inspired features or cleanup targets.
- After confirmation, implement only Chapter A: no-flash preloaded screenshot page and hotkey entry, then stop for user build/run latency feedback before Chapter B.

### Chapter 168 Sub-Agent Review Merge

- Sub-agent review identified additional current bottleneck hypotheses: screenshot backup file writing can still sit before payload emission, frontend image decode/ready gates can delay `overlay_ready_to_show`, and save output cache invalidation should use explicit selection/annotation versions rather than incidental ref dependencies.
- Snow Shot review confirmed the highest-value reference patterns: precreated hidden 1x1 draw window, disabled DWM transitions, parallel capture/window preparation, WebView2 SharedBuffer, Flatbush/RTree candidate lookup, UIAutomation child candidates, multi-monitor bounding box, separate fast save settings, and independent capture history assets.
- The detailed screenshot plan was updated to include these findings, plus a license caution: use Snow Shot as architectural reference only and do not copy GPL/commercial-licensed implementation code or resources.


### Chapter 168 Follow-Up - Screenshot Plan Expansion Review (2026-06-08)

### Goals

- Use sub-agents to compare Snow Shot, the current project, and the screenshot latency plan for missing optimization opportunities.
- Add professional product/architecture recommendations to the plan instead of only following the user's initial assumptions.
- Strengthen the ordinary Save As semantics for `Ctrl+S` and toolbar save: native Save dialog first, preserve screenshot state while dialog is open, exit only after successful write, stay in screenshot state on cancel or failure.
- Add a future about-panel plan with version, author, GitHub repository button, and contact email.
...
- Did not delete Snow Shot source or portable reference.
- Did not change save behavior, hotkey behavior, screenshot UI, or about page implementation yet.

### Validation

- Pending final docs check with `git diff --check` and keyword inspection.

### Next Recommended Chapter

- Let the user review the expanded plan, optionally using `$grill-me` to challenge priorities.
- If approved, start with Chapter 0 baseline instrumentation before changing behavior.


### Chapter 168 Follow-Up - Grill-Me Decision Lock (2026-06-08)

### Goals

- Use `$grill-me` and `$brainstorming` to turn the screenshot optimization plan into confirmed first-round design decisions.
- Respect the user's request to stop asking further questions after the confirmed decisions and use the recommended defaults for remaining minor choices.
- Keep this turn planning-only and avoid code implementation.

### Confirmed Decisions

- Screenshot wake-up must prioritize stability: no flicker, no stutter, repeatable behavior before raw milliseconds.
- First implementation round is limited to `Chapter 0 + A1 + A2 + B-lite + D1 + Chapter I`.
- Acceptance uses both subjective feel and measured data.
- Aggressive performance target is release hot-start `Alt+A -> interactive <150ms` and `Alt+A -> image ready <300ms`, but this is a target, not the Chapter 0/A1 failure gate.
- Ordinary `Ctrl+S` and toolbar Save remain Save As: native Save dialog first, cancel keeps screenshot state, successful write exits screenshot state and shows lightweight feedback.
- Fast Save is not part of the first round.
- Desktop empty-space candidate should be full monitor, not work area.
- First-round candidate priority is window/taskbar > monitor > visual candidate, with UIAutomation deferred.
- Taskbar should be considered as an overall window-like candidate in or after D1; Start button/taskbar area/tray segmentation is deferred.
- About page should reuse the existing route but replace old content with a lightweight product information card.
...
- Did not implement code changes.
- Did not create a separate spec document because the project documentation rules already index the screenshot plan as the accepted专项 planning document.
- Did not commit changes.

### Validation

- Pending final `git diff --check` and keyword inspection.

### Next Recommended Chapter

- If the user approves proceeding from this locked design, start with `Chapter 0`: baseline instrumentation only, with no behavior changes.


### Chapter 169 - Screenshot Baseline Instrumentation (2026-06-08)

### Goals

- Start the confirmed first implementation round with `Chapter 0`: baseline instrumentation only.
- Add stable timing logs for screenshot wake-up, frontend image loading/paint, overlay show, candidate first batch, and Save As without changing the intended screenshot behavior.
- Preserve the current Save As semantics and screenshot display order; this chapter is measurement-only, not an optimization chapter.
...
- Added a Rust screenshot session id like `ss-1` and backend `[screenshot-baseline]` phases: `hotkey_received`, `main_window_prepared`, `main_hidden_settled`, `capture_start`, `capture_end`, `backup_write_start`, `backup_write_end`, `window_reused`, `window_configured`, and `payload_emit`.
- Passed `sessionId` through the `screenshot-updated` payload so frontend timing logs can correlate with backend timing logs.
- Added frontend baseline phases for screenshot image loading: `frontend_session_start`, `file_load_start`, `file_load_end`, `image_decode_start`, `image_decode_end`, `mask_canvas_ready`, `first_paint`, `overlay_ready_to_show_called`, `overlay_ready_to_show_returned`, `analysis_image_data_ready`, `candidate_load_start`, and `candidate_first_batch`.
- Added an optional `session_id` to `overlay_ready_to_show` so the backend can log `overlay_show_result` for the same session.
- Added Save As baseline phases: `save_invoked`, `dialog_open_start`, `dialog_open_end`, `output_render_start`, `output_render_end`, `file_write_start`, `file_write_end`, and `overlay_exit_after_save`.

### Explicit Non-Goals

- Did not optimize or reorder screenshot capture, backup write, payload emission, image decode, overlay show, or candidate loading.
- Did not implement taskbar candidate, monitor candidate, UIAutomation, SharedBuffer, Fast Save, or about page changes.
- Did not change the ordinary Save As product semantics beyond adding logs.

### Validation

- `cargo fmt` from `tauri-client/src-tauri`: completed.
- `cargo check` from `tauri-client/src-tauri`: passed.
- `npm run build` from `tauri-client`: passed. Existing Vite warnings remain: mixed static/dynamic import for `@tauri-apps/api/window.js` and chunk size over `1200 kB`.
- `git diff --check`: passed. Only existing LF/CRLF warnings were reported by Git.

### Next Recommended Chapter

- User should run the app and press `Alt+A` repeatedly in dev or release to collect baseline logs.
- After reviewing the measured bottleneck, proceed to `A1`: low-risk hot-path fixes, starting with any confirmed blocker such as backup write waiting before payload emission.

### Chapter 170 - First-Round Screenshot Stability Batch (2026-06-08)

### Goals

- Execute the approved first-round scope in one larger implementation pass: `Chapter 0 + A1 + A2 + B-lite + D1 + Chapter I`.
- Prioritize stable repeatable behavior over risky pure-speed shortcuts: no flicker, no obvious stutter, cancel-safe Save As, and predictable candidate priority.
- Keep ordinary screenshot save as Save As only: `Ctrl+S` and toolbar save open the native Save dialog, cancel keeps the screenshot state, successful save exits screenshot state and shows lightweight feedback.
...
- `tauri-client/src/pages/ScreenshotPage.tsx`
- `tauri-client/src/types/screenshot.ts`
- `tauri-client/src/utils/detectionCandidates.ts`
- `docs/IMPLEMENTATION_CHAPTERS.md`

### What Changed

- A1 hot path: moved fullscreen backup image writing into a background task so payload emission and overlay readiness no longer wait for the temp file write.
- A1 session stability: propagated backend `sessionId` through memory/file/base64 payload paths and converted `overlay_ready_to_show` session ids to strings to avoid Tauri argument type mismatches.
- A2 frontend smoothness: delayed native candidate loading slightly after first paint so the overlay becomes visible before heavier candidate work starts.
- B-lite Save As: `Ctrl+S` and toolbar save now lower the screenshot overlay from always-on-top before opening the native Save dialog, restore focus/always-on-top when cancelled, and close the screenshot only after a successful write.
- B-lite feedback: successful save emits `screenshot-save-completed` before closing the overlay; the main app shows a compact success notification with the saved path and an Ant Design `打开文件夹` button.
- B-lite correctness: fixed Windows folder parsing for saved paths containing backslashes and expanded the output cache signature to include translated content, not only translated text length.
- D1 candidates: added taskbar-aware native candidates and full-display candidates, with priority kept as window/control/taskbar before display before visual for the first round.
- Chapter I about page: replaced the previous about content with a lightweight product information card containing product name, version, author, GitHub link, and contact email.

### Explicit Non-Goals

- Did not implement Fast Save; the first round intentionally keeps Save As semantics.
- Did not implement UIAutomation candidate detection; native window/taskbar/display plus visual fallback remain the first-round candidate stack.
- Did not implement shared-memory/raw-buffer screenshot transfer yet; memory payload still falls back to `get_fullscreen_image` base64 transfer, so this remains the likely next deep performance chapter if measured latency is still too high.
- Did not delete Snow Shot source/reference directories because they remain useful for future comparison.
- Did not commit changes.
...
- The largest remaining Alt+A latency source is still capture + PNG encode + base64 IPC + frontend decode/canvas work. If release hot-start remains above target, the next serious optimization should replace base64 IPC with a lower-copy image path rather than adding more small patches.
- Background backup writes still target the legacy temp backup path; the active overlay uses memory state, but future fallback debugging should consider session-specific backup filenames to avoid rapid-repeat overwrite confusion.
- Success feedback is shown from the restored main app window. If the product later requires true desktop/system toast behavior, add an owned notification path instead of relying only on in-window Ant Design notification.

### Next Recommended Chapter

- Rebuild and test release behavior with repeated `Alt+A`, `Ctrl+S` Save As success, Save As cancel, desktop empty-area candidate, taskbar candidate, and the About page.
- Collect `[screenshot-baseline]` logs for 5-10 repeated hot starts. If `get_fullscreen_image`, `image_decode`, or `mask_canvas_ready` dominates, start the next chapter on low-copy screenshot image transport and first-paint canvas splitting.

### Chapter 171 - Save Toast And Brand Surface Refinement (2026-06-08)
...
- `tauri-client/src/pages/SaveToastPage.tsx`
...
- Did not add a system tray or OS-native notification dependency.
- Did not change the Save As dialog semantics.
- Did not implement a reusable notification system beyond the focused save-success toast.
- Did not commit changes.
...
- The `219ms` toast measurement includes native Save dialog confirmation and window creation overhead; further reduction would require prewarming/reusing a toast window.
- The About page visual was validated by build and source inspection; final subjective polish should still be reviewed by the user in the running app.

### Next Recommended Chapter

- If the user wants the save toast to feel even closer to instant, prewarm a hidden reusable `save_toast` WebView at startup and update/show it instead of creating a new window after each save.

### Chapter 172 - Final Save Pill And Original UI Cleanup (2026-06-08)
...
- A settings-panel toggle for enabling/disabling save success hints is intentionally deferred per user instruction and should be handled later.


### Chapter 173 - Screenshot Hot Path Binary Transfer And Save-As Hardening (2026-06-08)

### Goals

- Continue the original screenshot latency task without adding the deferred save-toast settings toggle.
- Reduce Alt+A hot-path overhead while preserving no-flash, no-stutter, repeatable behavior.
- Keep Ctrl+S / toolbar save as Save As, with cancel staying in screenshot state and success exiting screenshot state.
- Add low-risk save-flow hardening discovered by review.
...
- Added `get_fullscreen_image_bytes` using `tauri::ipc::Response` so the screenshot overlay can fetch the captured PNG as an optimized array buffer instead of forcing a large base64 JSON IPC response.
- Changed the screenshot loader to create a Blob URL from binary PNG bytes and keep base64 only as a fallback path.
- Added baseline logs for `binary_fetch_end` and fallback `base64_fetch_end`, making the remaining hot-path transfer cost visible.
- Skipped redundant hidden screenshot-window hide calls when the prewarmed screenshot window is already hidden.
- Changed the hidden-main-window path so an already hidden/parked main window no longer pays the extra DWM flush + 16ms settle delay; the settle remains for the riskier path where a visible main window was just hidden before capture.
- Added `.png` extension hardening for Save As output paths, including the case where the user manually removes the extension.
- Renamed the save toolbar tooltip from generic save to Save As semantics: Chinese `??? Ctrl+S`, English `Save As Ctrl+S`.

### External Findings

- Tauri official documentation states that normal command return values are serialized to JSON and this can slow large payloads; for optimized array-buffer responses, `tauri::ipc::Response` is the recommended path.
- This matches the screenshot hot path because the app was returning multi-megabyte PNG data through base64 JSON IPC after the Rust capture had already completed.

### Validation

- `cargo check` from `tauri-client/src-tauri`: passed.
- `npm run build` from `tauri-client`: passed. Existing Vite warnings remain for mixed static/dynamic import and large chunk size.
- `cmd /c build.bat --no-pause` from repo root: passed and rebuilt the portable release.
- Release log smoke for one Alt+A: binary path was used, with `binary_fetch_end=90ms`, `first_paint=170ms`, `overlay_ready_to_show_returned=196ms`, and no base64 fallback log.
- Release Alt+A multi-run from `tmp-runtime-logs/release-alt-multi.log`:
  - `ss-1`: capture `66ms`, payload `70ms`, binary fetch `92ms`, first paint `172ms`, overlay returned `199ms`, candidates `250ms`.
  - `ss-2`: capture `64ms`, payload `69ms`, binary fetch `96ms`, first paint `170ms`, overlay returned `176ms`, candidates `231ms`.
  - `ss-3`: capture `54ms`, payload `58ms`, binary fetch `94ms`, first paint `167ms`, overlay returned `173ms`, candidates `226ms`.
  - `ss-4`: capture `79ms`, payload `89ms`, binary fetch `99ms`, first paint `173ms`, overlay returned `179ms`, candidates `233ms`.
  - `ss-5`: capture `55ms`, payload `60ms`, binary fetch `95ms`, first paint `168ms`, overlay returned `174ms`, candidates `226ms`.
- Release window-visibility automation also observed hot repeated overlay visibility at `60ms`, `83ms`, `86ms`, `87ms`, and `62ms` after an initial warm run, but the structured baseline log above is the preferred source for acceptance because it comes from app phases.
- Release Save As automation using keyboard flow created `tmp-runtime-logs/codex-save-as-space.png`, then exited screenshot state; after save, there were no large visible app windows, only the independent `?????` toast plus hidden helper windows.

### Known Risks

- `binary_fetch_end` still costs roughly `90-99ms` on the current 2560x1440 capture because the PNG is about 9MB; deeper optimization would require changing the transport/representation further, such as rendering from shared file/asset protocol or reducing PNG encode/decode cost.
- The first visible-main-window capture path still keeps the DWM settle to avoid reintroducing screenshots that include the app window or flicker.
- The save-success toast settings toggle remains intentionally deferred per user instruction.

### Next Recommended Chapter

- If more screenshot smoothness is needed, optimize the frontend first-paint stage next: reduce decode/draw overhead, pause hidden-window RAF when idle, and consider a non-PNG or lower-copy transfer path after measuring against Snow Shot behavior.


### Current Progress Summary - Screenshot Latency And Save Flow (2026-06-08)

### Scope Completed

- Completed cleanup of obsolete workspace artifacts requested by the user, including old handoff/planning files and test images, while preserving Snow Shot reference materials for future comparison.
- Established `docs/SCREENSHOT_LATENCY_AND_SELECTION_PLAN.md` as the detailed step-by-step screenshot latency and selection plan, indexed by the master plan.
- Implemented the first screenshot performance and UX pass covering Chapter 0, A1, A2, B-lite, D1, and Chapter I scope.
- Preserved the deferred requirement that the save-success hint toggle should be implemented later in the settings panel, not in the current round.

### Screenshot Startup Progress

- Prewarmed and reused the screenshot WebView instead of creating it cold during every Alt+A trigger.
- Kept the overlay hidden until the image is decoded and first paint is ready, which removed the earlier visible flash.
- Moved backup screenshot writing out of the visible startup path so it no longer blocks overlay readiness.
- Added detailed phase logs for hotkey received, capture start/end, payload emit, binary fetch, first paint, overlay show, and candidate loading.
- Added binary screenshot transfer via `tauri::ipc::Response`, replacing the previous multi-megabyte base64 IPC hot path while retaining base64 fallback.
- Reduced redundant hidden-window operations and skipped the extra DWM settle when the main window was already hidden or parked.

### Selection And Detection Progress

- Added full-display candidates so desktop/empty-space hover can still select a monitor-sized region.
- Added taskbar whole-window candidates before display and visual candidates.
- Preserved candidate priority as window/control/taskbar first, display second, visual fallback last.
- Deferred UI Automation sub-control detection to a later chapter to protect first-round stability and startup latency.

### Save Flow Progress

- Changed Ctrl+S and toolbar save to Save As semantics.
- Save As cancel keeps the screenshot state active and restores overlay focus/topmost state.
- Save success exits screenshot state and does not restore or pop the main window.
- Added an independent `save_toast` window for save success feedback, separate from the main window.
- Final save-success toast style is a compact white pill with a green check and `????` only.
- Added `.png` extension hardening when users remove the extension in the Save As dialog.
- Updated toolbar wording to `??? Ctrl+S` / `Save As Ctrl+S`.

### Main UI And About Page Progress

- Simplified the sidebar brand to a styled `Ysn Trans` wordmark.
- Removed the old About page content and rebuilt it into a centered product information panel.
- Current About page focuses on `????`, `Screenshot Translator`, and the short product description requested by the user.

### Latest Release Validation

- `cargo check` from `tauri-client/src-tauri`: passed.
- `npm run build` from `tauri-client`: passed.
- `cmd /c build.bat --no-pause` from repo root: passed and rebuilt the portable release.
- Latest release Alt+A structured baseline over five sessions:
  - `ss-1`: capture `66ms`, payload `70ms`, binary fetch `92ms`, first paint `172ms`, overlay returned `199ms`, candidates `250ms`.
  - `ss-2`: capture `64ms`, payload `69ms`, binary fetch `96ms`, first paint `170ms`, overlay returned `176ms`, candidates `231ms`.
  - `ss-3`: capture `54ms`, payload `58ms`, binary fetch `94ms`, first paint `167ms`, overlay returned `173ms`, candidates `226ms`.
  - `ss-4`: capture `79ms`, payload `89ms`, binary fetch `99ms`, first paint `173ms`, overlay returned `179ms`, candidates `233ms`.
  - `ss-5`: capture `55ms`, payload `60ms`, binary fetch `95ms`, first paint `168ms`, overlay returned `174ms`, candidates `226ms`.
- Window-visibility automation also observed repeated hot overlay visibility at `60ms`, `83ms`, `86ms`, `87ms`, and `62ms` after warmup.
- Save As automation created `tmp-runtime-logs/codex-save-as-space.png`; after save, screenshot state exited and no large main window appeared.

### Remaining Work

- The next latency bottleneck is now image representation/transfer/decode/first-paint, not cold screenshot window creation.
